#!/usr/bin/env python3
""" Take a single photo and upload to idokep.hu weather service """
...
def log_camera_settings(camera, bright=None):
    ...
    logging.info('CSV, CAM, %s, exp_mode= %s, framerate= %0.2f, iso= %d, shutter_speed= %d, '
                 + 'digital_gain= %0.2f, analog_gain= %0.2f, exposure_speed= %d, brightness= %d',
...
def increase_percent(base, prc):
    return int(base + base * prc / 100)
...
def adjust_camera_exp_mode(camera, bright, expo_state):
    ...
    Warning: Tests showed that there is a problem with increasing shutter speed with
             small amounts. At a certain level the shutter speed is doubled:
             shutter speed before: 1871384 (1.87 sec)
               + increase with 50 --> 
             shutter speed after : 3742865 (3.74 sec)

             Similar strange things happen when decreasing shutter speed.
             shutter speed before: 3746729 (3.74 sec)
               decrease with 10000 -->
             shutter speed after : 5107671 (5.1 sec) ! So even more, altough decreased.
             
             TL;DR: Setting a shutter speed between 1.8 sec and 3.7 sec can result in strange
             (doubled) values.
    """
    ...
    logging.info("cropped brightness=%d, shutterspeed=%d", bright, camera.shutter_speed)
    ...
    if bright < 20:
        if not manual_expo_mode(expo_state):
            ...
            # initialize shutter speed with exposure speed + x%
            camera.shutter_speed = increase_percent(camera.exposure_speed, incr_ss_percent)
            logging.info("set manual_expo_mode, shutterspeed=%d", camera.shutter_speed)

            skip_current_loop = True
        elif camera.shutter_speed < int((max_ss) * 1000 * 1000):
            camera.shutter_speed = increase_percent(camera.shutter_speed, incr_ss_percent)
            logging.info("            increased shutterspeed=%d", camera.shutter_speed)
    ...
    logging.debug(" skip this loop: %b", skip_current_loop)
    ...
    with picamera.PiCamera() as camera:
        try:
            ...
            while stay_in_loop:
                ...
                stay_in_loop = adjust_camera_exp_mode(camera, bright, expo_state)
                # exit While loop

            # save #FIXME save only after while loop
            img.save(filename, quality=qual, optimize=True)
            os.system("gpicview " + filename)
